...
// 偏好标签 — 按维度分组
const PREF_GROUPS: { group: string; icon: React.ReactNode; tags: { key: string; label: string }[] }[] = [
  {
    group: "时间效率",
  ...
  {
    group: "出行方式",
    icon: <Train size={13} strokeWidth={1.8} />,
    tags: [
      { key: "公交方便",     label: "公交方便" },
      { key: "少换乘",       label: "少换乘" },
      { key: "步行citywalk", label: "步行 citywalk" },
      { key: "自驾出行",     label: "自驾出行" },
    ],
  },
  {
    group: "消费偏好",
    icon: <Wallet size={13} strokeWidth={1.8} />,
    tags: [
      { key: "人少景美",     label: "人少景美" },
      { key: "热门打卡",     label: "热门打卡" },
      { key: "小众特色",     label: "小众特色" },
...
const AVOID_TAGS: { key: string; label: string }[] = [
  { key: "人多",       label: "人太多" },
  { key: "商业街",     label: "商业街过度喧闹" },
  { key: "贵",         label: "消费太贵" },
  { key: "需排队",     label: "排队超1小时" },
  { key: "辣",         label: "辣的食物" },
  { key: "太远",       label: "距离太远" },
  { key: "体力消耗大", label: "体力消耗大" },
];
...
export function OnboardingPage({ onDone }: OnboardingPageProps) {
  ...
  function handleFinish() {
    ...
    setSaving(true);
    // 1.2 秒仪式感动画 → 0.3 秒淡出 → 进入首页
    setTimeout(() => setFadingOut(true), 1200);
    setTimeout(() => {
      setSaving(false);
      onDone(profile);
    }, 1500);
  ...
  return (
    <div className={`ob-shell${fadingOut ? " ob-fade-out" : ""}`}>
      {/* 仪式感加载动画遮罩 */}
      {saving && (
      ...
      <div className="ob-progress-wrap">
        <div className="ob-progress-bar" style={{ ['--progress-width' as string]: `${(step / TOTAL_STEPS) * 100}%` }} />
        <span className="ob-progress-label">{step} / {TOTAL_STEPS}</span>
      </div>
      ...
      <div className="ob-content">
              ...
              {SCENARIOS.map(({ key, icon, label, desc }) => (
                ...
                >
                  ...
                  {scenarios.includes(key) && (
                    <span className="ob-scenario-check">
        ...
        {step === 2 && (
          <div className="ob-step ob-step-enter">
            <div className="ob-section-label">
              <span className="ob-section-dot primary" />
              我更看重
            </div>

            {PREF_GROUPS.map(({ group, icon, tags }) => (
              <div key={group} className="ob-pref-group">
                <div className="ob-pref-group-title">
                  {icon}
                  {group}
                </div>
                <div className="ob-tag-grid">
                  {tags.map(({ key, label }) => (
                    ...
                    >
                      {prefs.includes(key) && <Check size={11} strokeWidth={2.5} />}
                      {label}
                    </button>
                  ))}
                  {group === "体验风格" && (
            ...
            {/* 🚫 我想避开 */}
            <div className="ob-section-label" style={{ marginTop: 28 }}>
              <span className="ob-section-dot danger" />
              我想避开
            </div>
            <div className="ob-tag-grid">
              {AVOID_TAGS.map(({ key, label }) => (
                <button
                  key={key}
                  type="button"
