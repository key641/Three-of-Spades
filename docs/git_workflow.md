...
每次合并到 `main` 前，都要判断是否需要更新 `docs/version_log.md`。

必须更新的情况：

- 新增功能或改变用户操作流程。
- 改了接口字段、schema、mock 数据结构或前后端契约。
- 改了 Agent、路线策略、评分、重规划、反馈等核心逻辑。
- 改动会影响其他同学负责的模块。
- 引入临时开关、mock、兼容逻辑或需要后续清理的技术债。
...
1. 完成功能改动
2. 跑对应验证命令
3. 在 docs/version_log.md 追加本次记录
4. 开 PR 时勾选 PR 模板里的版本文档检查项
5. Review 时由另一个同学确认记录是否写清楚影响范围
```

如果不确定要不要写，默认写一条短记录。文档短一点没关系，但队友不知道你改了什么会很痛。
