...
class AgentOrchestrator:
    ...
    async def handle_message(
            ...
            if min_dist is not None and min_dist <= 8.0:
                ...
                trace.append(
                    AgentTraceStep(
                        ...
                        label=f"使用 GPS 坐标作为起点：({request.start_lat:.4f}, {request.start_lng:.4f})",
                        status="done",
                        details={
                            "start_lat": request.start_lat,
                            "start_lng": request.start_lng,
                            "min_dist_to_poi_km": round(min_dist, 1),
        ...
        async def build_routes(candidate_pois: list[POI], allow_min_stops_fallback: bool) -> list[Route]:
            ...
            for role in required_roles:
                role_candidates = [poi for poi in candidate_pois if role in poi.route_roles][:4]
                selected_ids.update(poi.id for poi in role_candidates)
        ...
        for recall_limit, relax_preferences in [(None, False), (200, True), (260, True)]:
            ...
            if not final_pois:
                final_pois = pois
            attempt_routes = await build_routes(pois, allow_min_stops_fallback=False)
            recall_attempts.append(
                {
                    "limit": recall_limit,
                    "relax_preferences": relax_preferences,
                    "poi_count": len(pois),
                    "route_count": len(attempt_routes),
                }
            )
            if len(attempt_routes) > len(routes):
                ...
                [
                    {
                        ...
                        "content": (
                            ...
                            "✨ 推荐方案：方案一必须明确写“首推”，并说明它为什么最适合本轮需求；方案二、方案三要说明各自不同的取舍或侧重点。"
                            "每条单独一行，格式为“方案一｜路线标题（首推）：亮点；约X公里，交通约Y分钟，人均约Z元”。"
                            ...
                            "stop.reason、highlight_text、ugc_tip 只提炼为简短亮点；transport_mode_from_previous、distance_km_from_previous 用于说明明确的交通信息；字段为空时跳过，不要编造。"
    ...
    def _gps_city_from_request(self, request: ChatRequest | None) -> str:
        ...
        return self.clarification_policy._coords_to_city(lat, lng) if lat and lng else ""
    ...
    async def _parse_intent(
        ...
        if prefetched is not None:
            trace.append(AgentTraceStep(step="parse_intent", label="LLM 解析用户意图（并行预取）", status="done", details=self._intent_trace_details(prefetched)))
            return prefetched
        try:
            ...
            if gps_city:
                intent = intent.model_copy(update={"city": gps_city})
            trace.append(AgentTraceStep(step="parse_intent", label=f"LLM 解析失败，使用 fallback：{type(exc).__name__}", status="fallback", details=self._intent_trace_details(intent)))
    ...
    async def _llm_parse_intent(self, message: str) -> Intent:
        schema_fields = {
            "city": "string, 无法从消息推断时返回空字符串，不要猜测默认城市",
            ...
            "city_from_message": "boolean, 仅当消息明确提到或可推断城市时为 true",
        }
        response = await self.llm_client.complete(
            [
                {
                    ...
                    "content": (
                        ...
                        "city_from_message 仅在消息明确提到或可推断城市时设为 true。"
                    ),
                },
                {
                    "role": "user",
    ...
    def _normalize_intent_data(self, data: dict) -> dict:
        ...
        for key in ("start_time", "scenario"):
        ...
        for key in ("need_clarification", "city_from_message"):
            value = normalized.get(key, defaults.get(key, False))
            normalized[key] = value.strip().lower() in {"true", "1", "yes", "y", "是", "需要"} if isinstance(value, str) else bool(value)
